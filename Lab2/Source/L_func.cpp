	...
	{	lisp s;
		...
		if(isdigit(x) || (x == '-' || x == '+') && isdigit(c)){ 
			...
			s->node.atom.ch = x;
				...
				ungetc(c, stdin);
				ungetc(x, stdin);
				fgets(s->node.atom.trig, 4, stdin);
				s->node.atom.tag = 2;
			}
		}
		return s;
...
//  а без обрамляющих скобок - write_seq
	...
	if (isNull(x)) cout << "()";
			...
			if(x->node.atom.tag == 0){
			cout << ' ' << x->node.atom.num;
			return;
			}
			if(x->node.atom.tag == 1){
			cout << ' ' << x->node.atom.ch;
			return;
			}
			if(x->node.atom.tag == 2){
			cout << ' ' << x->node.atom.trig;
			return;
			}
		}
